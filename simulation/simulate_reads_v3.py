...
def usage(s=None):
	message = """
usage: [cat <fasta_file> |] simulate_reads [options]
...
def main():
	global nucToSubstitutions,fixedStrand,qualityChar
	global debug
	...
	for arg in argv[1:]:
		...
		if (arg.startswith("--insert=")):
			...
			if (argVal == 0) or (argVal == "none"):
				...
				noiseProb = parse_probability(argVal)
		elif (arg == "--lower"):
			showAsLower = True
		elif (arg.startswith("--indel=")):
			if (argVal == 0) or (argVal == "none"):
				gapOpenProb = gapExtendProb = None
			else:
				(gapOpenProb,gapExtendProb) = argVal.split(",",1)
				gapOpenProb   = parse_probability(gapOpenProb)
				gapExtendProb = parse_probability(gapExtendProb)
				assert (gapExtendProb < 1)
	...
	if (insertLength == None):
		...
		for readNum in xrange(numReads):
			...
			(basesNeeded,cigar) = random_cigar(readLength,gapOpenProb,gapExtendProb)
	...
	#=== generate paired reads ===
		...
		for readNum in xrange(numReads):
			...
			(basesNeeded1,cigar1) = random_cigar(readLength[0],gapOpenProb,gapExtendProb)
			(basesNeeded2,cigar2) = random_cigar(readLength[1],gapOpenProb,gapExtendProb)
			...
			if (noiseProb != None):
				seq1 = random_subs(seq1,noiseProb)
				seq2 = random_subs(seq2,noiseProb)
			if (gapOpenProb != None):
...
def write_sam_header(f,sampler):
	...
	print >>f, "@PG	ID:simulate_reads\tPN:simulate_reads"
